...
session_start();
if (isset($_SESSION['user_id']) && $_SESSION['totalprice']) {
    $totalprice = $_SESSION['totalprice'];
    $user_id = $_SESSION['user_id'];

    $finduser = mysqli_query($con, "SELECT * FROM `user` WHERE `id` = '$user_id'");
    if ($row = mysqli_fetch_array($finduser)) {
        ...
        if ($err) {
            ...
            if (isset($res->success) && $res->success == '1') {
                ...
                // echo "<a href = 'payment-success.php'> payment </a>";
                header('Location:' . $payUrl);
            }
        }
    }
} else {
    echo 'you enter wrong page';
    header('index.php');
}
